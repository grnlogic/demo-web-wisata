...
export default async function BeritaPage() {
  ...
  let content = {
    hero: {
      ...
      subtitle: "Sorotan wisata, event, kuliner, dan pengembangan daerah yang dikurasi dengan visual rapi, bukan portal generik.",
      stats: {
        trend: "Tren harian",
  ...
  if (lang === "en") {
    ...
    const [trContent, trBerita] = await Promise.all([
      ...
      Promise.all(berita.map(async (item) => ({
        ...item,
        judul: await translateText(item.judul, "id", "en"),
        ringkasan: item.ringkasan ? await translateText(item.ringkasan, "id", "en") : null,
  ...
  return (
    <BeritaClient initialBerita={beritaFormatted as any} content={content} lang={lang} />
  );
}
