...
export default function EditBeritaPage() {
  ...
  const fetchBerita = async () => {
    try {
      const response = await fetch(`/api/admin/berita/${beritaId}`);
      ...
      setFormData({
        ...
        tags: berita.tags || [],
        sourceUrl: berita.sourceUrl || "",
        sourceImage: berita.sourceImage || "",
        isExternal: berita.isExternal,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(error.message || "Terjadi kesalahan saat mengupdate berita");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="mt-4 text-slate-600">Memuat data berita...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">Perbarui informasi berita</p>
        </div>
        <button
