...
export default function EditUKMPage() {
  ...
  const fetchUKM = async () => {
    try {
      const response = await fetch(`/api/admin/ukm/${params.id}`);
      if (response.ok) {
        const data = await response.json();
        setFormData({
          namaUsaha: data.namaUsaha || "",
          deskripsi: data.deskripsi || "",
          kategori: data.kategori || "Kerajinan",
        ...
        setGambar(data.gambar && data.gambar.length > 0 ? data.gambar : [""]);
        setProdukLayanan(
          data.produkLayanan && data.produkLayanan.length > 0
            ? data.produkLayanan
            : [""]
        );
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        alert(`Gagal memperbarui UKM: ${data.error}`);
      }
    } catch (error) {
      console.error("Error updating UKM:", error);
      alert("Terjadi kesalahan saat memperbarui UKM");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-800">Edit UKM</h1>
            <p className="text-slate-600 mt-1">Perbarui informasi profil UKM</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            ...
            <span>{loading ? "Menyimpan..." : "Perbarui UKM"}</span>
          </button>
        </div>
      </form>
