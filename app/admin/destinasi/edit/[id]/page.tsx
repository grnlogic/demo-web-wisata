...
export default function EditDestinasiPage({
  ...
  const [images, setImages] = useState<DestinasiImage[]>([
    { url: "", caption: "", isPrimary: true },
  ]);

  const [harga, setHarga] = useState<DestinasiHarga[]>([
    { jenisHarga: "", harga: "", keterangan: "" },
  ]);

  const [fasilitas, setFasilitas] = useState<DestinasiDetail[]>([
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          jumlahReview: data.jumlahReview?.toString() || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        alert(`Gagal mengupdate destinasi: ${data.error}`);
      }
    } catch (error) {
      console.error("Error updating destinasi:", error);
      alert("Terjadi kesalahan saat mengupdate destinasi");
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-slate-600">Memuat data destinasi...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Form - Menggunakan komponen yang sama dengan Create */}
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Fasilitas & Detail - Same as Create */}
        <div className="bg-white rounded-xl p-6 shadow-lg space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-slate-800">
              Fasilitas & Detail
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg space-y-4">
          ...
          <div className="flex items-center space-x-4 pt-4 border-t border-slate-200">
            ...
            >
              ...
              <span>{saving ? "Menyimpan..." : "Update Destinasi"}</span>
            </button>
